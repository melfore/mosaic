...
import DocsTitle from "./DocumentationTitle";
...
const DocsPage: FC<IDocsPage> = ({ basedOn, component, e2eTestInfo, localizableProps }) => {
  return (
    <div className={DOCUMENTATION_BODY_CLASS}>
      ...
      <DocsTitle text="Canvas" subtitle />
      <Primary />
      <ArgsTable story={PRIMARY_STORY} />
      <DocsTitle text="Usage" />
      <p>
      ...
      {e2eTestInfo && (
        <Fragment>
          <DocsTitle text="E2E Testing" subtitle />
          <p>
          ...
          <p>
            By design <code className={DOCUMENTATION_CODE_LINE_CLASS}>data-cy</code> attribute is not mandatory for the
            final user, but required for e2e testing, so when not explicitely provided it is defaulted to:{" "}
          ...
          {localizableProps && e2eTestInfo.dataCyShortcut && (
            <p>
              ...
              with the value of the <code className={DOCUMENTATION_CODE_LINE_CLASS}>{e2eTestInfo.dataCyShortcut}</code>{" "}
              property.
            </p>
          )}
